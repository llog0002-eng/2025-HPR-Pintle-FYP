...
Ps_f = Pc*1.2; % fuel supply pressure (bar)
Ps_ox = 45; % oxidiser supply pressure (bar)
...
t_post = 2; % thickness of post (mm)
t_pintle = 8; % thickness of pintle tip (mm)
...
Cd_i = 0.5; % MIT use 0.5 for cavitation https://wikis.mit.edu/confluence/display/RocketTeam/Modeling
Cd_o = 0.8; 
...
A_o = mf/(Cd_o*sqrt(2*rho_f*dP_f))*(1e3)^2; % Outer orifice area (mm2)
A_i = 20; % mox/(Cd_i*sqrt(2*rho_ox*dP_ox))*(1e3)^2*throttle; % Inner orifice area (mm2)
Gap_o = sqrt(A_o/pi+(Dpt/2)^2)-Dpt/2; % Outer flow opening distance (mm)
Gap_i = A_i/(pi*Dpt); % Pintle opening distance (mm)
...
k_dyer = sqrt((Ps_ox-Pc)/(Pv_ox-Pc)); % Dyer model non-equilibrium parameter
