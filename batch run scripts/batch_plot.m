...
title('Comparison of Washers');
grid on;

% Fit a linear model to the data for each washer
hold on; % Ensure we are plotting on the same figure
colors = lines(4); % Define colors for each washer

% Initialize legend entries
legendEntries = cell(1, 4);

for i = 1:4
    x = [eval(sprintf('washer%d', i)).average_after_max_p]; % Accessing the average pressure
    y = [eval(sprintf('washer%d', i)).mean_Q_after_max]; % Accessing the mean flowrate
    p = polyfit(x, y, 1); % Linear fit
    yfit = polyval(p, x); % Evaluate the fit
    ...
    % Print R-squared value and trendline equation in the command window
    fprintf('Washer %d: R^2 = %.2f, y = %.2fx + %.2f\n', i, rsq, p(1), p(2));
    
    % Update the legend to include trendline information
    throttle_percentage = [100, 78.6, 47.4, 36.7]; % Define throttle percentages
    legendEntries{i} = sprintf('%.1f%% Throttle', throttle_percentage(i));
    
    % Update the legend to include trendline information with equations
    legendEntries{i+4} = sprintf('%.1f%% Throttle, y = %.2fx + %.2f', ...
         throttle_percentage(i), p(1), p(2));
end
...
title('Cd vs Pressure');
legend(legendEntries{1:4}, 'Location', 'northeast');
grid on;
%cd vs flowraate
figure;
